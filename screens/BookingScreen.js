...
const BookingScreen = () => { 
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        const colRef = query(collection(db, "bookings"), orderBy("createdAt", "desc"));
...
console.log("Today Date", new Date().toLocaleTimeString());
        ...
        >
            <View style={styles.container}>
                {isLoading ? (
                    ...
                    <View>
                        {/* if there allBookings equals to 0 then show this message */}
                        {allBookings.length === 0 ? (
                            <View style={{ flex: 1, alignItems: "center", marginTop: 100 }}
                            >
                                <Text style={{ color: "green", fontSize: 22, }}
                                > Vous n'avez pas de réservations</Text>
                                </View>
                                ) : (
                                    <View >
                                    {allBookings.filter((Bookings) => Bookings.userID === currentUserID).map((item, indexi) => (
